...
function activate(context) {
    const cmd = vscode.commands.registerCommand('twigPreview.openPreview', () => {
        ...
        const detectVariables = (tpl, baseDir = null, visited = new Set()) => {
            const result = {};
            ...
            while ((m = varRegex.exec(tpl)) !== null) {
                ...
                if (loopMap[parts[0]]) {
                    ...
                    if (!arrayProps[arrName])
                        arrayProps[arrName] = new Set();
                    if (parts.length === 1) {
                        continue;
                    }
                    else {
            ...
            while ((m = ifRegex.exec(tpl)) !== null) {
                const name = m[1].trim();
                if (name === 'not' || name === 'empty' || name.includes(' '))
                    continue;
            ...
            for (const arrName in arrayProps) {
                ...
                for (let i = 0; i < pathParts.length; i++) {
                    ...
                    if (i === pathParts.length - 1) {
                        if (cur[p] === undefined)
                            cur[p] = [itemObj];
                        else if (Array.isArray(cur[p]) && cur[p].length === 0)
                            cur[p].push(itemObj);
                    }
                    else {
                        if (cur[p] === undefined || typeof cur[p] !== 'object')
        ...
        const inlineIncludes = (template, baseDir, visited = new Set()) => {
            const collected = {};
            if (!baseDir)
                return { content: template, vars: {} };
            const includeRegex = /{%\s*include\s+['\"]([^'\"]+)['\"](?:\s+with\s+(\{[^}]*\}))?\s*%}/g;
            const out = template.replace(includeRegex, (match, incPath, withPart) => {
                try {
                    // parse withPart
                    if (withPart) {
                        try {
        ...
        const renderTemplate = (vars = {}) => {
            ...
            try {
                ...
                safeFns.forEach((fn) => {
                    try {
                        Twig.extendFunction(fn, function () { return ''; });
                    }
                    catch (e) { /* ignore */ }
                });
            }
            ...
            const registerNoopForFunctions = (template) => {
                try {
            ...
            const registerTemplates = (template, baseDir, visited = new Set()) => {
                if (!baseDir)
                    return template;
...
function getWebviewContent(initialVars = '{}') {
    return `<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
